  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const vids = await getCachedCollection<Video>(db, "videos", [where("courseId", "==", courseId)], `course_${courseId}`);
            ...
            {filtered.map((video) => (
              ...
              >
                ...
                <div className="px-3 py-2.5 sm:p-3">
                  ...
                  <p className="text-xs text-muted-foreground mt-1"><p className="text-xs text-muted-foreground mt-1">{settings.appName || "LMS"}</p></p>
                </div>
              </button>
            ))}
          </div>
