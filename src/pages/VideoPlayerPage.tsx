  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const allVids = await getCachedCollection<Video>(db, "videos", [where("courseId", "==", v.courseId)], `course_${v.courseId}`);
        if (cancelled) return;
        const vids = allVids
  ...
  return (
    <button onClick={() => navigate(`/video/${v.id}`)}
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-muted-foreground mt-0.5"><p className="text-xs text-muted-foreground mt-0.5">{settings.appName || "LMS"}</p></p>
      </div>
    </button>
  );
}
