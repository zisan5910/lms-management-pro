...
export default function AdminDataPage() {
  ...
  // Course-wise full backup export
  const handleCourseBackupExport = async () => {
    if (!selectedCourseExport) { toast.error("Select a course first"); return; }
    setExporting("course-backup");
    try {
      const course = courses.find(c => c.id === selectedCourseExport);
      const courseId = selectedCourseExport;
      ...
      // Filter by course
      const courseUsers = usersSnap.docs
        .map(d => ({ id: d.id, ...d.data() }))
        .filter((u: any) => u.enrolledCourses?.some((c: any) => c.courseId === courseId) || u.activeCourseId === courseId);

      const courseVideos = videosSnap.docs
        .map(d => ({ id: d.id, ...d.data() }))
        .filter((v: any) => v.courseId === courseId);

      const courseEnrollRequests = enrollReqSnap.docs
        .map(d => ({ id: d.id, ...d.data() }))
        .filter((r: any) => r.courseId === courseId);

      const courseExams = examsSnap.docs
        .map(d => ({ id: d.id, ...d.data() }))
        .filter((e: any) => e.courseId === courseId);

      const examIds = courseExams.map((e: any) => e.id);
      const courseSubmissions = submissionsSnap.docs
        .map(d => ({ id: d.id, ...d.data() }))
        .filter((s: any) => examIds.includes(s.examId));

      const backupData = {
        course: course ? { id: course.id, ...course } : null,
        users: courseUsers,
        videos: courseVideos,
        enrollRequests: courseEnrollRequests,
        exams: courseExams,
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `course-backup-${course?.courseName?.replace(/\s+/g, '-') || courseId}-${new Date().toISOString().slice(0, 10)}.json`;
      a.click();
      URL.revokeObjectURL(url);
      toast.success(`Course backup exported: ${courseUsers.length} users, ${courseVideos.length} videos, ${courseExams.length} exams`);
    } catch (err: any) { toast.error(err.message || "Export failed"); }
    setExporting(null);
  };
  ...
  return (
    <div className="p-4 max-w-2xl mx-auto animate-fade-in">
      ...
      <div className="p-4 bg-card rounded-lg border border-border mb-4">
        <h3 className="font-medium text-foreground mb-3">📦 Course-wise Backup</h3>
        <p className="text-xs text-muted-foreground mb-3">Export all users, videos, exams, and submissions for a specific course.</p>
        <select value={selectedCourseExport} onChange={e => setSelectedCourseExport(e.target.value)}
          className="w-full px-3 py-2.5 rounded-lg bg-background border border-border text-foreground text-sm mb-3">
          <option value="">Select a course</option>
          {courses.map(c => <option key={c.id} value={c.id}>{c.courseName}</option>)}
        </select>
        <button
          onClick={handleCourseBackupExport}
          disabled={!selectedCourseExport || !!exporting}
          className="w-full flex items-center gap-2 px-4 py-2.5 rounded-lg bg-primary text-primary-foreground text-sm font-medium disabled:opacity-50 justify-center"
        >
          <Download className="h-4 w-4" />
          {exporting === "course-backup" ? "Exporting..." : "Export Course Backup"}
        </button>
      </div>
